...
class TransUNetSegmenter(BaseSegmenter):
    ...
    def _initialize_segmentation_network(self):
        ...
        transformer_config.patches.grid = (
            self.configs['image_size'] // vit_patches_size,
            self.configs['image_size'] // vit_patches_size
        )
        segmentation_network = VisionTransformer(
            transformer_config,
            img_size=self.configs['image_size'],
            num_classes=transformer_config.n_classes
        )

        if not self.configs.get("fine_tune"):
            pretrained = self.configs.get("pretrained_path", None)
            if pretrained and os.path.exists(pretrained):
                segmentation_network.load_from(weights=np.load(pretrained))
            else:
                print(f"[WPI] pretrained_path not found, skipping backbone init: {pretrained}")
