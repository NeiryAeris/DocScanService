...
class AnalysisSegmenter:
    ...
    def __init__(self, model_checkpoint: str, device: str, class_to_color_map: Union[str, Path],
        ...
        # Keep an explicit path in config for components that expect a filename (e.g., SegmentationPlotter)
        self.config['class_to_color_map'] = str(class_to_color_map)
    ...
    def load_network(self) -> BaseSegmenter:
        ...
        # Ensure class_to_color_map points to an existing file (fix relative-path issues)
        cmap = self.config.get("class_to_color_map", None)
        candidate = Path(__file__).resolve().parent.parent / "handwriting_colors.json"
        if (not cmap) or (not Path(cmap).is_file()):
            if candidate.is_file():
                self.config["class_to_color_map"] = str(candidate)

        lightning_model = get_segmenter_class(self.config).load_from_checkpoint(self.config['fine_tune'], configs=self.config)
